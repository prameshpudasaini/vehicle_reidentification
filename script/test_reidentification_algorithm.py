...
def evaluatePerformance(param_model):
    ...
    print("MAPE on test set: {:.4f}".format(mape_test_full))
    print("RMSE on test set: {:.4f}".format(rmse_test_full), "\n")
    
    # make predictions on candidate adv for matching
    X_adv_candidate = cdf.drop(['file', 'adv', 'stop', 'match', 'travel_time'], axis = 1)
    y_pred_adv_candidate = param_model.predict(X_adv_candidate)
    
    return {'mape_valid': mape_valid,
            'rmse_valid': rmse_valid,
            'mape_test': mape_test,
            'rmse_test': rmse_test,
            'mape_test_full': mape_test_full,
            'rmse_test_full': rmse_test_full,
            'y_pred_adv_candidate': y_pred_adv_candidate}
...
ridge_reg_tuned = Ridge(**ridge_grid_result['best_params'])

# evaluate performance
ridge_reg_result = evaluatePerformance(ridge_reg_tuned)

# add predicted travel time values
cdf_ridge = cdf.copy(deep = True)
cdf_ridge['y_pred'] = ridge_reg_result['y_pred_adv_candidate']
...
# function to get predicted travel time for individual vehicle actuation over adv det
def predictTravelTime(df_pred_tt, file, id_adv):
    X_i = df_pred_tt.copy(deep = True)[(df_pred_tt.file == file[:-4]) & (df_pred_tt.adv == id_adv)] # discard .txt
    X_i.reset_index(drop = True, inplace = True)
    tt_predict = X_i.loc[0, 'y_pred'] # predicted travel time
    return tt_predict
...
def reidentifyAlgorithm(df_pred_tt):
            ...
            for j in id_stop_look:
                ...
                if stop_time > adv_time: # look forward in timestamp
                    ...
                    if tt_thru_min <= tt_adv_stop <= tt_thru_max:
                        tt_predict = predictTravelTime(df_pred_tt, file, i) # predicted travel time
    ...
    match_result_class = pd.concat([match_TP, match_FP, match_FN])
    match_result_metrics = {'TP': match_TP.shape[0],
                            'FP': match_FP.shape[0],
                            'FN': match_FN.shape[0]}
    
    return {'match_result_class': match_result_class,
            'match_result_metrics': match_result_metrics}

ridge_result = reidentifyAlgorithm(cdf_ridge)