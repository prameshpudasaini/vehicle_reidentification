    ...
    for i in id_adv:
        ...
        for j in id_stop_look:
            ...
            if stop_time > adv_time: # look forward in timestamp
                ...
                tt_expected = 2*(dist_det - len_adv) / (adv_vel + stop_vel) # ideal travel time
                tt_error = abs(tt_adv_stop - tt_expected)
...
def computeReidMetrics():
    ...
    if eff_length_adv == 26: # calibrated effective vehicle length
        match_result.to_csv("ignore/reidentification_result_analytical/match_result_Lu_method.txt", sep = '\t', index = False)
...
metrics.to_csv("ignore/reidentification_result_analytical/metrics_Lu_method.txt", sep = '\t', index = False)